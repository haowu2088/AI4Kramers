"""
load the weights of encoder from autoencoder model and train classifier model on the fine-tuning datasets.
...
import torch.nn as nn                              # Wu: new from pretrain
from torch.utils.tensorboard import SummaryWriter   # Wu: new from pretrain
import numpy as np                # Wu: seems no use later
import matplotlib.pyplot as plt   # Wu: seems no use later
from accelerate import Accelerator
from accelerate import DistributedDataParallelKwargs  # Wu: new from pretrain
from kogger import Logger

import config as cfg
from dataset import TrainCIFData   # Wu: different than pretrain (PretrainCIFData)
...
def main():
    ...
    # optimizer = optim.SGD(model.parameters(), config['lr'], momentum=config['momentum'], weight_decay=config['weight_decay'])        # Wu: notice!
    ...
    if config['load_pretrain']:
        ...
        accelerator.load_state(input_dir=config['pre_ckpt_path'])    # Wu: notice
    elif config['continuous_train']:
        if accelerator.is_main_process:
            logger.info('Continue train...')
        accelerator.load_state(input_dir=config['ckpt_path'])       # Wu: notice
    train(
