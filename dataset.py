...
At the first time, CIF file will be read into memory and processed into tensor data, which will be saved in disk. 
At the next time, tensor data will be read into memory from disk directly without processing CIF file.

@reference: https://github.com/txie-93/cgcnn    # Wu: useful!
"""

import csv   # read from and write to CSV files
import os
import os.path as osp
import random    #  generate random numbers, pick random items from a list, shuffle sequences randomly, and other such functionalities
...
class TrainCIFData(Dataset):
    ...
    def __getitem__(self, idx):
        if idx < self.len_posi_fake:
            ...
            atom_fea, nbr_fea, nbr_fea_idx, degree, target, cif_id = self.dataset_nega[0][idx], self.dataset_nega[1][idx], self.dataset_nega[2][idx], self.dataset_nega[3][idx], self.dataset_nega[4][idx], self.dataset_nega[5][idx]
...
class PredictCIFData(Dataset):
    def __init__(self, root_dir, processed_dir, logger, max_num_nbr, radius, dmin, step):
        self.root_dir                 = root_dir
        self.processed_dir_cand       = osp.join(self.root_dir, processed_dir)    # Wu: notice
        self.logger                   = logger
        self.max_num_nbr, self.radius = max_num_nbr, radius
        assert os.path.exists(root_dir), 'root_dir does not exist!'
