...
import json    #  parsing JSON formatted data into Python objects and vice versa
...
def collate_pool(dataset_list):
    ...
    Parameters
    ----------
    dataset_list: list of tuples for each data point.     (atom_fea, nbr_fea, nbr_fea_idx, target)  # Wu: cif_id?
    ...
    batch_cif_id = []
    base_idx = 0
    for i, ((atom_fea, nbr_fea, nbr_fea_idx, degree), target, cif_id) in enumerate(dataset_list):
    ...
    return (torch.cat(batch_atom_fea, dim=0),
            ...
            crystal_atom_idx), torch.stack(batch_target, dim=0), batch_cif_id
    ...
    for i in tqdm(range(length)):
        ...
        for nbr in all_nbrs:
            ...
            if len(nbr) < max_num_nbr:
                ...
                fea = list(map(lambda x: x[1], nbr)) + [radius + 1.] * (max_num_nbr - len(nbr))
